...
impl Bitboard {
    ...
    pub const fn shift_one(self, direction: Direction) -> Self {
        ...
        Bitboard(
            self.0.rotate_left(Bitboard::SHIFT[direction])
            & Bitboard::AVOID_WRAP[direction]
        )
    }
    ...
    pub const fn sliding_attack(self, occlusion: Bitboard, direction: Direction) -> Self {
        ...
        Bitboard(next.shift_one(direction).0 | next.0)
    }
    ...
    pub fn square(&self) -> u8 {
        debug_assert!(self.0.count_ones() == 1, "Bitboard must have exactly one bit set to get the square index.");
        self.0.ilog2() as u8
    }
}
